...
impl TerrainGenerator {
  ...
  pub fn generate(
    ...
    num_levels: i32,
    base_level: f64,
  ) -> Vec<Vec<CellTerrainInfo>> {
    let mut terrain = vec![vec![CellTerrainInfo::default(); width]; height];
    ...
    for y in 0..height {
      for x in 0..width {
        ...
        for level in 0..num_levels {
          ...
          altitude += self.altitude_perlin.get([
            x as f64 / (base_level / scale as f64),
            y as f64 / (base_level / scale as f64),
          ]) as f32
            / (num_levels as f32);
        }
        let temperature = self
          .temperature_perlin
          .get([x as f64 / 20.0, y as f64 / 20.0])
          as f32;

        terrain[y][x] = CellTerrainInfo {
          altitude,
          temperature,
        };
      }
    ...
    for row in terrain {
      for cell in row {
        if let Some(value) = cell.get(index) {
          ...
          let char_index =
            (normalized * (ascii_gradient.len() - 1) as f32).round() as usize;
